...
std::vector<std::vector<int>> transponer_matriz(const std::vector<std::vector<int>>& matriz) {
    int filas = matriz.size();
    int columnas = matriz[0].size();
    std::vector<std::vector<int>> matriz_transpuesta(columnas, std::vector<int>(filas));
...
std::vector<std::vector<int>> multiplicar_matrices_optimizado(const std::vector<std::vector<int>>& A, const std::vector<std::vector<int>>& B_T) {
    int n = A.size();
...
void procesar_archivo(const std::string& nombre_archivo) {
    ...
    // Medir el tiempo de ejecución de la multiplicación considerando la transposición
    auto inicio = std::chrono::high_resolution_clock::now();

    // Transponer la segunda matriz
    std::vector<std::vector<int>> matriz2_transpuesta = transponer_matriz(matriz2);

    std::vector<std::vector<int>> matriz_resultante = multiplicar_matrices_optimizado(matriz1, matriz2_transpuesta);

    auto fin = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> duracion = fin - inicio;
    
    std::cout << "Tiempo de ejecucion para " << nombre_archivo << " (optimizado): " << duracion.count() << " segundos.\n";

    // Guardar la matriz resultante en un archivo
    std::string nombre_resultado = "matriz_resultante_optimizado_" + std::to_string(filas) + "x" + std::to_string(columnas) + ".txt";
